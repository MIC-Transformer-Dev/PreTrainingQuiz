...
function VerifyMail(props) {
	...
	const sendCode = async () => {
		...
		if (userType === "organizer") {
			...
			url = `https://localhost:3000/admin/resendverificationemail`;
		} else {
			// url = `https://quizzie-api.herokuapp.com/user/resendverificationemail`;
			url = `https://be-quizzie.herokuapp.com/user/resendverificationemail`;
		}
	...
	const handleSubmit = async () => {
		...
		if (!errors && codeError.length === 0) {
			...
			if (userType === "organizer") {
				...
				url = `https://be-quizzie.herokuapp.com/admin/verifyEmail`;
			} else if (userType === "user") {
				// url = `https://quizzie-api.herokuapp.com/user/verifyEmail`;
				url = `https://be-quizzie.herokuapp.com/user/verifyEmail`;
			}
